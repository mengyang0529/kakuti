...
const PDFViewerContent = ({ file, documentId, onMarkdownUpdate }) => {
  ...
  // 防重复调用的ref
  const isProcessingWandSelect = useRef(false)
  ...
  const handleWandSelect = useCallback(({ pageIndex, minX, maxX, pathBounds, anchor, pageRect }) => {
    ...
    // 防重复调用 - 如果正在处理中，则忽略
    if (isProcessingWandSelect.current) {
      console.log('handleWandSelect: already processing, ignoring duplicate call')
      return
    }
    
    // 立即设置处理标志
    isProcessingWandSelect.current = true
    console.log('handleWandSelect: processing started')
    
    // 使用setTimeout确保异步重置，避免同步调用问题
    setTimeout(() => {
      isProcessingWandSelect.current = false
      console.log('handleWandSelect: processing flag reset')
    }, 100)
    ...
    // Convert Magic Wand coordinates to page-relative coordinates
    const pageRelativeMinY = pathBounds.minY - pageRect.top
    const pageRelativeMaxY = pathBounds.maxY - pageRect.top
    
    console.log('Magic Wand coordinate conversion:', {
      originalMinY: pathBounds.minY,
      originalMaxY: pathBounds.maxY,
      pageRectTop: pageRect.top,
      pageRelativeMinY: pageRelativeMinY,
      pageRelativeMaxY: pageRelativeMaxY
    })
    ...
    const selection = calculateCorridorSelection({
      ...
      minX,
      maxX,
      minY: pageRelativeMinY,
      maxY: pageRelativeMaxY
    })
    
    if (!selection.isEmpty) {
      // 将Magic Wand选择结果传递给useHighlight系统
    ...
    console.log('handleWandSelect: processing completed')
  }, [viewerRef, triggerActionInputDialog, handleMagicWandSelection])
  ...
  }, [contextMenu.show, clearAllPaths])
  ...
  const highlightOverlays = useMemo(() => {
    const overlays = getHighlightOverlays()
    if (overlays.length > 0) {
      console.log('HighlightOverlays generated:', overlays.length, 'overlays from', highlights.length, 'highlights')
    }
    return overlays
